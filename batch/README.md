...
4. **Results** are automatically merged into `data/leads.md`, processed offers are reconciled out of the `data/inbox.md` inbox, and integrity is verified with `verify-ledger.mjs` at the end of the run.
...
4. After all workers finish, batch-runner calls `merge.mjs` to merge TSVs into `data/leads.md`, `reconcile.mjs` to move processed offers out of the `data/inbox.md` inbox, and `verify-ledger.mjs` to check integrity.
