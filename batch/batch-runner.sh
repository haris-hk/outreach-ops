...
# outreach-ops batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
outreach-ops batch runner — process job offers in batch via claude -p workers
...
merge_tracker() {
  ...
  node "$PROJECT_DIR/engine/merge.mjs"
  echo ""
  echo "=== Reconciling pipeline.md ==="
  node "$PROJECT_DIR/engine/reconcile.mjs" || echo "⚠️  Pipeline reconcile had issues (see above)"
  echo ""
  echo "=== Verifying pipeline integrity ==="
  node "$PROJECT_DIR/engine/verify-ledger.mjs" || echo "⚠️  Verification found issues (see above)"
}
...
watch_status() {
  ...
  # Chain verify-ledger.mjs
  if [[ -f "$PROJECT_DIR/engine/verify-ledger.mjs" ]]; then
    echo ""
    echo "=== Running pipeline verification ==="
    node "$PROJECT_DIR/engine/verify-ledger.mjs" || echo "⚠️  Verification found issues"
  fi
}
...
main() {
  ...
  echo "=== outreach-ops batch runner ==="
  if (( LIMIT > 0 )); then
