...
/**
 * doctor.mjs — Setup validation for outreach-ops
...
function checkPlaywrightMcp(root) {
  ...
  return {
    ...
    fix: [
      ...
      'Tracking: https://github.com/harishussainkhan/outreach-ops/issues/506',
    ],
  };
}
...
const USER_LAYER_PREREQS = [
  {
    ...
    fix: [
      'Run onboarding (modes/onboard.md) or: cp templates/profile/background.example.md profile/background.md',
      'Then fill in who you are / what the business does, with proof points',
    ],
  },
  {
    path: 'profile/offer.yml',
    fix: [
      'Run: cp templates/profile/offer.example.yml profile/offer.yml',
      'Then edit what you sell or the role you seek',
    ],
  },
  {
    path: 'profile/icp.yml',
    fix: [
      'Run: cp templates/profile/icp.example.yml profile/icp.yml',
      'Then define your target segments and triggers',
    ],
  },
  {
    path: 'profile/preferences.yml',
    fix: [
      'Run: cp templates/profile/preferences.example.yml profile/preferences.yml',
      'Then set channels, tone, cadence, never-contact list',
    ],
  },
];
...
Paste job URLs below as \`- [ ] {url}\` then run \`/outreach-ops pipeline\`.
...
function onboardingState(root) {
  const autoCopied = [];
  const templates = [
    { target: 'profile/preferences.yml', template: 'templates/profile/preferences.example.yml' },
    { target: 'profile/voice-dna.md', template: 'templates/voice-dna.example.md' },
  ];
