...
const SCRIPT = join(ROOT, 'engine', 'followup-seed.mjs');
...
function run(args, sandbox, extraEnv = {}) {
  const env = {
    ...process.env,
    OUTREACH_OPS_TRACKER: sandbox.tracker,
    OUTREACH_OPS_FOLLOWUPS: sandbox.followups,
    OUTREACH_OPS_FOLLOWUPS_LOCK: sandbox.lock,
    ...extraEnv,
  };
  try {
...
// ── Test 9: lock held by a live pid → exit 4 ────────────────────────────────
{
  ...
  const res = run(['1'], sb, { OUTREACH_OPS_FOLLOWUPS_LOCK_TIMEOUT_MS: '200' });
