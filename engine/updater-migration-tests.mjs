...
let source = '';
try {
  source = readFileSync(new URL('./update-system.mjs', import.meta.url), 'utf-8');
...
const twoPassManifestChecks = [
  {
    ...
    pattern: /OUTREACH_OPS_UPDATE_REEXEC/,
  },
  {
  ...
  },
  {
    ...
    pattern: /OUTREACH_OPS_UPDATE_BACKUP_BRANCH/,
  },
  {
...
for (const userPath of ['profile/', 'data/', 'campaigns/', 'plugins.local/', 'plugins.lock', 'config/plugins.yml']) {
