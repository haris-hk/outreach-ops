 ...
 *   OUTREACH_OPS_FOLLOWUPS_LOCK              lock directory override
 *   OUTREACH_OPS_FOLLOWUPS_LOCK_TIMEOUT_MS   lock acquire timeout
 *   OUTREACH_OPS_FOLLOWUPS_LOCK_RETRY_MS     lock retry interval
 *   OUTREACH_OPS_FOLLOWUPS_LOCK_STALE_MS     stale-lock recovery threshold
 */
...
const FOLLOWUPS_LOCK_PREFIX = 'outreach-ops-followups-';
...
function resolveFollowupsPath(override) {
  if (override) return override;
  if (process.env.OUTREACH_OPS_FOLLOWUPS) return process.env.OUTREACH_OPS_FOLLOWUPS;
  return join(OUTREACH_OPS, 'data/follow-ups.md');
}
...
function resolveLockDir(explicitLockDir, followupsPath) {
  ...
  return resolveFollowupsLockDir(process.env.OUTREACH_OPS_FOLLOWUPS_LOCK, lockKey);
}
...
export async function seedFollowup(appNum, options = {}) {
  ...
  const lock = await acquireFollowupsLock(lockDir, followupsPath, {
    timeoutMs: options.lockTimeoutMs ?? envInt('OUTREACH_OPS_FOLLOWUPS_LOCK_TIMEOUT_MS', 60_000),
    retryMs: options.lockRetryMs ?? envInt('OUTREACH_OPS_FOLLOWUPS_LOCK_RETRY_MS', 75),
    staleMs: options.lockStaleMs ?? envInt('OUTREACH_OPS_FOLLOWUPS_LOCK_STALE_MS', 10 * 60_000),
  });
