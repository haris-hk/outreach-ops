 ...
 *
 * Usage:
 *   node outreach-ops/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN]
 *
 ...
 * or outside the outreach-ops repository.
 *
...
export function injectPrintPageCss(html, format = 'a4') {
  ...
  const pageStyle = `<style id="outreach-ops-page-setup">\n@page { size: ${pageSize}; margin: ${PDF_PAGE_MARGIN}; }\n</style>`;
 ...
 * outreach-ops root with forward slashes. One row per PDF path; when a report
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.outreach-ops-render-${randomUUID()}.html`);
