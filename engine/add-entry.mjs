#!/usr/bin/env node
/**
 * add-entry.mjs — Deterministic dedup + insertion for `/outreach-ops add`.
 *
 ...
 *
 * Test isolation: OUTREACH_OPS_CV / OUTREACH_OPS_ARTICLE_DIGEST override the target
...
const CV_FILE = process.env.OUTREACH_OPS_CV || join(OUTREACH_OPS, 'cv.md');
const ARTICLE_DIGEST_FILE = process.env.OUTREACH_OPS_ARTICLE_DIGEST || join(OUTREACH_OPS, 'article-digest.md');
...
export function applyAdd(payload, { cvText = null, articleText = null } = {}) {
  ...
  if (payload.articleDigest) {
    ...
    const current = articleText === null
      ? '# Article Digest -- Proof Points\n\nCompact proof points from portfolio projects. Read by outreach-ops at evaluation time.\n'
      : articleText;
