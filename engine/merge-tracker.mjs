...
const ADDITIONS_DIR = process.env.OUTREACH_OPS_ADDITIONS
  ? process.env.OUTREACH_OPS_ADDITIONS
  : join(OUTREACH_OPS, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
const DRY_RUN = process.argv.includes('--dry-run');
const VERIFY = process.argv.includes('--verify');
const MIGRATE = process.argv.includes('--migrate');
const MERGE_HOLD_MS = Number(process.env.OUTREACH_OPS_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.OUTREACH_OPS_MERGE_READY_IPC === '1';
 ...
 *
 * `OUTREACH_OPS_TRACKER_LOCK` exists for tests and unusual local layouts, but the
 ...
 * and use the outreach-ops lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `outreach-ops-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('outreach-ops-merge-tracker-')) return fallback;
  return candidate;
}
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.OUTREACH_OPS_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.OUTREACH_OPS_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.OUTREACH_OPS_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
  });
...
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === OUTREACH_OPS ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
