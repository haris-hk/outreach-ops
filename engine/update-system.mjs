...
/**
 * update-system.mjs — Safe auto-updater for outreach-ops
 *
...
export const DEFAULT_GIT_TIMEOUT_MS = parsePositiveInt(process.env.OUTREACH_OPS_GIT_TIMEOUT_MS, 30000);
export const DEFAULT_GIT_FETCH_TIMEOUT_MS = parsePositiveInt(
  process.env.OUTREACH_OPS_GIT_FETCH_TIMEOUT_MS,
  Math.max(DEFAULT_GIT_TIMEOUT_MS, 300000),
);
export const NPM_INSTALL_TIMEOUT_MS = parsePositiveInt(process.env.OUTREACH_OPS_NPM_INSTALL_TIMEOUT_MS, 60000);
export const PLAYWRIGHT_INSTALL_TIMEOUT_MS = parsePositiveInt(process.env.OUTREACH_OPS_PLAYWRIGHT_INSTALL_TIMEOUT_MS, 120000);
export const DASHBOARD_REBUILD_TIMEOUT_MS = parsePositiveInt(process.env.OUTREACH_OPS_DASHBOARD_REBUILD_TIMEOUT_MS, 60000);
export const UPDATE_PATH_CHECKOUT_BUDGET_MS = parsePositiveInt(process.env.OUTREACH_OPS_UPDATE_PATH_CHECKOUT_BUDGET_MS, 5000);
export const REEXEC_BUFFER_TIMEOUT_MS = parsePositiveInt(process.env.OUTREACH_OPS_REEXEC_BUFFER_TIMEOUT_MS, 60000);
...
function gitTimeoutEnvVar(args) {
  return args[0] === 'fetch' ? 'OUTREACH_OPS_GIT_FETCH_TIMEOUT_MS' : 'OUTREACH_OPS_GIT_TIMEOUT_MS';
}
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: outreach-ops-update-checker',
    ]),
  ]);
...
async function apply() {
  ...
  const isReexec = process.env.OUTREACH_OPS_UPDATE_REEXEC === '1';
  ...
  try {
    ...
    const backupBranch = process.env.OUTREACH_OPS_UPDATE_BACKUP_BRANCH || updateBackupBranchName(local);
    if (!isReexec) {
      try {
    ...
    if (!isReexec) {
      ...
      try {
        ...
        execFileSync(process.execPath, ['update-system.mjs', 'apply'], {
          ...
          env: {
            ...process.env,
            OUTREACH_OPS_UPDATE_REEXEC: '1',
            OUTREACH_OPS_UPDATE_BACKUP_BRANCH: backupBranch,
          },
        });
        return;
      } catch (err) {
