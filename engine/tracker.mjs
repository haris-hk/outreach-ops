...
// OUTREACH_OPS_TRACKER_DB could point both names at the same file).
...
function loadStates() {
  if (!existsSync(STATES_PATH)) {
    console.error(`Error: ${STATES_PATH} not found — cannot validate statuses. Run from the outreach-ops root.`);
    process.exit(1);
  }
