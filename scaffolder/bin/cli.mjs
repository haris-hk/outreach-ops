#!/usr/bin/env node
// outreach-ops scaffolder — one-command install.
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "outreach-ops-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  const target = dirArg || "outreach-ops";
  ...
  // Pretty path for messages: "./outreach-ops" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning outreach-ops${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  console.log(`\n✓ outreach-ops is ready in ${display}\n`);
