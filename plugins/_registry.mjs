...
/** Find a registry entry by bare id, full name, or `outreach-ops-plugin-<id>`. */
export function findInRegistry(root, nameOrId) {
  const reg = loadRegistry(root);
  return reg.plugins.find(p =>
    p.id === nameOrId || p.name === nameOrId || p.name === `outreach-ops-plugin-${nameOrId}`) || null;
}
...
export function validateRegistryEntry(e, { idRe, hookKinds, reservedEnv }) {
  const errs = [];
  if (typeof e.name !== 'string' || !e.name.startsWith('outreach-ops-plugin-')) errs.push('name must start with "outreach-ops-plugin-"');
  if (typeof e.id !== 'string' || !idRe.test(e.id)) errs.push('invalid id');
  if (e.name && e.id && e.name !== `outreach-ops-plugin-${e.id}`) errs.push('name must equal outreach-ops-plugin-<id>');
