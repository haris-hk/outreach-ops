...
> Claude Code marketplace metadata). These plugins extend outreach-ops itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with outreach-ops) or
...
outreach-ops is plain ESM with no build step, so the engine **cannot truly
